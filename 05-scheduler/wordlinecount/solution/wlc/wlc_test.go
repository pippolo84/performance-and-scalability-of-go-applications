...
func TestWordCountWordCountSeq(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.descr, func(t *testing.T) {
			got := WordLineCountSeq([]io.Reader{strings.NewReader(tc.haystack)}, tc.needle)
			if got != tc.want {
...
func TestWordCountWordCountConc(t *testing.T) {
	for _, tc := range testCases {
		t.Run(tc.descr, func(t *testing.T) {
			got := WordLineCountConc([]io.Reader{strings.NewReader(tc.haystack)}, tc.needle, runtime.NumCPU())
			if got != tc.want {
				t.Fatalf("WordLineCountConc(%q, %q)\ngot: %d\nwant: %d", tc.haystack, tc.needle, got, tc.want)
			}
		})
	}
}
